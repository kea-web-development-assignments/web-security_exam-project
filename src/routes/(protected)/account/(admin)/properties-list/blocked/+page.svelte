...
<svelte:head>
	<title>Blocked properties | Airbnb</title>
</svelte:head>

<h1 class="[grid-area:header] text-4xl font-bold">Blocked properties</h1>
    ...
    {#if $page.data.properties.length}
        {#each $page.data.properties as property}
            <form
                ...
                method="POST"
                action="?/unblockProperty"
                use:enhance={() => {
                    return async ({ result, update }) => {
                        if(result.status === 200) {
                            await invalidateAll();
                            alert('Property has been unblocked.');
                            return;
                        }
            ...
            >
                ...
                <PropertyCard {property}>
                    <section class="flex justify-end text-lg" slot="actions">
                        <input
                            class="w-2/5 bg-rose-500 text-white text-center rounded-lg cursor-pointer p-2 mt-2"
                            type="submit"
                            value="Unblock"
                        >
                    </section>
